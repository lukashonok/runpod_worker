...
def handler(job):
    ...
    def test_and_get_gdrive_service(google_folder_id, creds_json_str):
        """
        Authenticates with Google Drive using service account credentials and performs a pre-flight check,
        supporting Shared Drives. Returns an authenticated service client or an error.
        """
        try:
            ...
            print(f"worker-comfyui - Google Drive auth pre-flight check for project_id: {creds_dict.get('project_id')}, client_email: {creds_dict.get('client_email')}")

            credentials = ServiceAccountCredentials.from_json_keyfile_dict(creds_dict, scope)
            
            # Build the service object, use cache_discovery=False to avoid issues in stateless environments
            service = build('drive', 'v3', credentials=credentials, cache_discovery=False)

            # Pre-flight check with supportsAllDrives=true
            service.files().get(fileId=google_folder_id, supportsAllDrives=True, fields='id').execute()
            
            return service, None # Return the authenticated service client
        except HttpError as e:
            if e.resp.status == 404:
                return None, f"Google Drive folder with ID '{google_folder_id}' not found or you don't have permission. (Shared Drive Hint: Ensure service account has access to the Shared Drive itself, not just the folder)."
            return None, f"Google Drive API error during pre-flight check: {e.resp.status} {e.reason}"
        except Exception as e:
            # Catch other potential errors like malformed JSON in creds
            return None, f"Google Drive authentication failed during pre-flight check: {str(e)}"
    ...
    # --- Google Drive Pre-flight Check ---
    google_folder_id = job_input.get("google_folder_id")
    drive_service = None
    if google_folder_id:
        creds_json_str_raw = os.environ.get("GOOGLE_DRIVE_CREDS")
        if not creds_json_str_raw:
            return {"error": "Job failed: GOOGLE_DRIVE_CREDS environment variable is not set."}
        try:
            creds_json_str = base64.b64decode(creds_json_str_raw).decode('utf-8')
        except Exception as e:
            return {"error": f"Job failed: Could not base64 decode GOOGLE_DRIVE_CREDS: {str(e)}"}
        
        drive_service, error_msg = test_and_get_gdrive_service(google_folder_id, creds_json_str)
        if error_msg:
            return {"error": error_msg}
        print("worker-comfyui - Google Drive authentication pre-flight check successful.")
    ...
    try:
        ...
        # ---------------------------------------------------------------------------
        # Custom: Handle Google Drive upload if google_folder_id is provided
        # ---------------------------------------------------------------------------
        if google_folder_id: # and implicitly drive_service is not None from pre-flight
            ...
            for node_id, node_output in outputs.items():
                if "images" in node_output:
                    for image_info in node_output["images"]:
                        ...
                        source_path = os.path.join(comfyui_output_dir, subfolder, filename)
                        if not os.path.exists(source_path):
                            errors.append(f"Source file '{source_path}' not found on disk.")
                            continue
                        
                        try:
                            media_body = MediaFileUpload(source_path, mimetype='image/png', resumable=True)
                            ...
                            uploaded_files.append({"file_id": file.get('id'), "file_name": filename})
                        except Exception as e:
                            errors.append(f"Failed to upload '{filename}' to Google Drive: {str(e)}")

            if errors:
                return {"error": "Job failed during Google Drive upload.", "details": errors, "uploaded_files": uploaded_files}

            print(f"worker-comfyui - Job completed. Uploaded {len(uploaded_files)} image(s) to Google Drive folder '{google_folder_id}'.")
            return {"status": "success", "uploaded_files": uploaded_files}
        
        # ---------------------------------------------------------------------------
        # Custom: Handle local file saving if 'save_to_path' is provided
        # ---------------------------------------------------------------------------
        save_to_path = job_input.get("save_to_path")
        if save_to_path:
            import shutil

            saved_files = []
            comfyui_output_dir = "/comfyui/output"  # Assumes standard ComfyUI output path
                ...
                if "images" in node_output:
                    for image_info in node_output["images"]:
                        ...
                        try:
                            ...
                            abs_save_path = os.path.abspath(save_to_path)
                            destination_file_path = os.path.abspath(os.path.join(abs_save_path, filename))
                            
                            if not destination_file_path.startswith(abs_save_path):
                                errors.append(f"Invalid destination path (directory traversal attempt): {filename}")
                                continue

                            os.makedirs(os.path.dirname(destination_file_path), exist_ok=True)
                            shutil.move(source_path, destination_file_path)
                            saved_files.append(destination_file_path)
                        except Exception as e:
                            errors.append(f"Failed to move '{filename}' to '{save_to_path}': {str(e)}")
            
            # After processing all images, return the result
            if errors:
                return {"error": "Job failed during file saving.", "details": errors, "saved_files": saved_files}
            
            print(f"worker-comfyui - Job completed. Moved {len(saved_files)} image(s) to '{save_to_path}'.")
